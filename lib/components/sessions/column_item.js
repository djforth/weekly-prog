...
var ColumnItem = function (_React$Component) {
  ...
  _createClass(ColumnItem, [{
    ...
    value: function _checkbtn(item) {
      ...
      var no_link = { link: null, title: null };
      if (!item.has('buttons')) return no_link;
      var btns = item.get('buttons');
    ...
    value: function _actions() {
      ...
      var _checkbtn2 = this._checkbtn(item);

      var link = _checkbtn2.link;
      var title = _checkbtn2.title;

      console.log(link, title);
      return React.createElement(BookBtn, { places: places, link: link, title: title });
    }
  }, {
    key: '_showContent',
